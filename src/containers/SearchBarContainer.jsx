...
export default function SearchBarContainer(props) {
	...
	const GetDamageRelation = async (types) => {
		...
		let objDamage = [];
		const arrType = types;
		const promises = arrType.map((type) =>
		...
		pokemons.map((pokemon, index) => {
			objDamage = {
				...
				doubleDamageFromLength:
					pokemon["damage_relations"]["double_damage_from"].length,
				doubleDamageTo: pokemon["damage_relations"]["double_damage_to"],
				doubleDamageToLength:
					pokemon["damage_relations"]["double_damage_to"].length,
				halfDamageFrom: pokemon["damage_relations"]["half_damage_from"],
				halfDamageFromLength:
					pokemon["damage_relations"]["half_damage_from"].length,
				halfDamageTo: pokemon["damage_relations"]["half_damage_to"],
				halfDamageToLength:
					pokemon["damage_relations"]["half_damage_to"].length,
				noDamageFrom: pokemon["damage_relations"]["no_damage_from"],
				noDamageFromLength:
					pokemon["damage_relations"]["no_damage_from"].length,
